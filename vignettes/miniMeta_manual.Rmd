...
(Mantel-Hanszel, Inverse Variance, Peto and GLMM) and heterogeneity estimators 
...
table cell, and the meta-analysis is updated. 

As this filling in of the table widget is relatively slow (especially over
a web connection), it can be turned off by unchecking 
the "Auto-update table and analysis" checkbox. When this is unchecked, the 
table widget becomes static and the meta-analysis is NOT updated as the data 
are adjusted. When it _is_ checked (the default), then with every table cell 
altered the data are checked (and potential problems are indicated with a 
message that appears below the table widget), the SE is recalculated based on
the 95% CI, and the meta-analysis is updated. If the "Auto-calculate SE" is 
unchecked, then the SE is NOT updated unless it is deleted explicitly.
